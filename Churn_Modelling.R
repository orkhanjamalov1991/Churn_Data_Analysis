...
#1. Import dataset.
view(Churn_Modelling_1_)
#2. Remove unneeded columns.
raw <- Churn_Modelling_1_
Churn_Modelling_1_$RowNumber <- NULL
Churn_Modelling_1_$CustomerId <- NULL
Churn_Modelling_1_$Surname <- NULL
#3. Build Churn model.
#4. Compare model results for training and test sets.
#5. Evaluate and explain model results using ROC & AUC curves.
Churn_Modelling_1_$Exited %>% table() %>% prop.table()
Churn_Modelling_1_$Exited <- Churn_Modelling_1_$Exited %>% 
  as.factor()

# ----------------------------- Data Preprocessing -----------------------------
Churn_Modelling_1_ %>% inspect_na()
...
#7. Create repository named “Churn_Data_Analysis” in your Github account and push your homework
#Script to this repository.
#8. Fork other users’ repositories, make pull requests (at least one, making three pull requests is
                                                       #desirable).
